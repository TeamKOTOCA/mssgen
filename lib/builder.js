...
const WEBP_BINARY_NAME = process.platform === 'win32' ? 'mssgen-webp.exe' : 'mssgen-webp';
const WEBP_BINARY_PATH = path.join(os.tmpdir(), 'mssgen', WEBP_BINARY_NAME);
const WEBP_CONVERTER_SOURCE_PATH = path.join(__dirname, 'webp-converter.c');
...
function getSourceFiles(rootDir) {
  return getWatchedFiles(rootDir)
    ...
    .map((file) => ({
      ...
      outputPath: path.join(rootDir, DIST_DIR_NAME, shouldConvertImageToWebp(file.relativePath)
        ? getWebpRelativePath(file.relativePath)
        : file.relativePath),
    }));
}
...
function ensureWebpConverter() {
  const sourceStat = fs.statSync(WEBP_CONVERTER_SOURCE_PATH);
  const binaryExists = fs.existsSync(WEBP_BINARY_PATH);
  const needsCompile = !binaryExists || fs.statSync(WEBP_BINARY_PATH).mtimeMs < sourceStat.mtimeMs;

  if (!needsCompile) {
    return WEBP_BINARY_PATH;
  }

  ensureDir(path.dirname(WEBP_BINARY_PATH));
  execFileSync('gcc', [
    '-O2',
    WEBP_CONVERTER_SOURCE_PATH,
    '-o',
    WEBP_BINARY_PATH,
    '-lpng',
    '-ljpeg',
...
function convertImageToWebp(sourcePath, outputPath) {
  const converterPath = ensureWebpConverter();
  execFileSync(converterPath, [sourcePath, outputPath]);
}
...
async function build(rootDir = process.cwd()) {
  ...
  for (const { absolutePath, outputPath, relativePath } of sourceFiles) {
    ...
    if (isTextFile(relativePath)) {
      ...
      convertImageToWebp(absolutePath, outputPath);
    } else {
