...
`setting.json` の `build.webpExclude` に配列でファイルパス（ビルドルート相対）を指定すると、その画像は変換対象外になります。WebP 変換と参照置き換えの対象は、ビルドルート配下の `**/assets/` にある `png` / `jpg` / `jpeg` に限定されます。

```json
{
  "build": {
    "excludeDirs": ["drafts", "nested/private"],
    "webpExclude": [
      "assets/no-convert.png",
      "nested/assets/hero.jpg"
    ]
  }
}
```
...
## 出力されるもの
mssgen はカレントディレクトリを再帰的に走査し、対象ファイルを `dist/` に同じ構造で出力します。`setting.json` の `build.rootDir`（または `build.sourceDir`）に相対パスを指定すると、そのディレクトリをビルドルートとして走査し、`common/` の読み込み、WebP 変換、URL 置き換え、`webpExclude` のパス判定もそのビルドルート起点で行います。`build.excludeDirs` にビルドルート相対のフォルダーパスを指定すると、そのフォルダー配下はビルド・監視対象から除外します。
...
- `mssgen help` / `mssgen --help` / `mssgen <command> --help`: ヘルプを表示
